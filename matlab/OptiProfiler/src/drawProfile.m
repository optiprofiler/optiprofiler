function [fig, ax] = drawProfile(x, y, perf_or_data, ratio_max, labels, tolerance_label)
    ...
    for i_solver = 1:n_solvers
        ...
        set(ax, 'XLim', [0.0, x_lim_max], 'YLim', [0.0, 1.0], 'TickLabelInterpreter', 'latex');

        if perf_or_data == 'perf'
            xt = get(ax, 'XTick');
            % arrayfun applies the function to each element of the array xt.
            xtl = arrayfun(@(x) ['$', '2^{', num2str(x), '}$'], xt, 'UniformOutput', false);
            set(ax, 'XTickLabel', xtl, 'TickLabelInterpreter', 'latex');
        end
    end
