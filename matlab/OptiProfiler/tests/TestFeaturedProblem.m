    ...
    methods (Test)
        ...
        function testSimple(testCase)
            ...
            for n = nValues
                ...
                testCase.verifyEqual(size(featured_problem.fun_values), [0 0]);
                testCase.verifyEqual(size(featured_problem.maxcv_values), [0 0]);

                % Evaluate the objective function at x0.
                f = featured_problem.fun(x0);
                testCase.verifyEqual(featured_problem.n_eval, 1);
                testCase.verifyEqual(featured_problem.fun_values, f);
