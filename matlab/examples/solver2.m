function x = solver2(fun, x0)
%SOLVER2 is a TOY SOLVER for demonstration. It will be used in the examples under the `examples` folder.
%
...
% In practice, you would replace this with a more sophisticated optimization algorithm.
    ...
    % solver2 will randomly sample 50 * n points in the search space and return the one with the lowest
    % function value, where n is the dimension of the problem.
    n = length(x0);
    n_eval_factor = 50;
    n_eval = n_eval_factor * n;
    rand_stream = RandStream('mt19937ar', 'Seed', 2);
    xhist = NaN(n, n_eval);
    fhist = NaN(n_eval, 1);
    for i = 1:n_eval
