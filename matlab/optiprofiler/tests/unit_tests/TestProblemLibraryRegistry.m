classdef TestProblemLibraryRegistry < matlab.unittest.TestCase
    methods (Test)
        ...
        function testBundledRootCannotBeOverriddenByRegistry(testCase)
            cleanup = isolateRegistry(testCase); %#ok<NASGU>
            injected_root = createProvider(testCase, 's2mpj', true);
            registrations = struct( ...
                'name', 's2mpj', ...
                'api_version', 1, ...
                'role', 'external', ...
                'root', injected_root, ...
                'select_function', 's2mpj_select', ...
                'load_function', 's2mpj_load', ...
                'collect_info_function', '', ...
                'check_available_function', '', ...
                'platforms', {{'linux', 'macos', 'windows'}});
            save(getenv('OPTIPROFILER_MATLAB_PROBLEM_LIBRARY_REGISTRY'), ...
                'registrations');

            library = resolveProblemLibrary('s2mpj');

            testCase.verifyEqual(library.role, 'bundled-default');
            testCase.verifyNotEqual(library.root, injected_root);
            testCase.verifyEqual(library.root, fileparts(which('s2mpj_load')));
        end
        ...
        function testResolvedProviderRunsOnParallelWorker(testCase)
            testCase.assumeTrue( ...
                logical(license('test', 'Distrib_Computing_Toolbox')), ...
                'A Parallel Computing Toolbox license is required.');
