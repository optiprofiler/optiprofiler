    ...
    methods (Test)
        ...
        function supersededFieldsAreRejectedNotReinterpreted(testCase)
            ...
            legacy = { ...
                ...
                setfield(valid, 'fun', 0), setfield(valid, 'maxcv', 0), setfield(valid, 'point', [1; 2]), ...
                setfield(valid, 'point', [])}; %#ok<SFLD>
        ...
        function unknownFieldsAndNakedValuesAreRejected(testCase)
            for name = {'x', 'optimum', 'value', 'merit_fun', 'Merit'}
                extended = setfield(TestProblemReference.record(), name{1}, 1); %#ok<SFLD>
        ...
        function validationNeverEvaluatesTheProblem(testCase)
            ...
            for k = 1:50
                with_reference.reference; %#ok<VUNUS>
            end
            testCase.verifyEqual([counter('fun'), counter('cub')], 2 * baseline);
        ...
        function allCustomKeySubsets(testCase)
            ...
            for b = 1:numel(builders)
                ...
                for mask = 0:(2^numel(keys) - 1)
                    subset = keys(logical(bitget(mask, 1:numel(keys))));
            ...
            for k = 1:numel(features)
                ...
                for variant = 1:2
                    ...
                    for repeat = 1:100  % reading the fact calls nothing
                        problem.reference; %#ok<VUNUS>
                        featured.reference; %#ok<VUNUS>
                    end
        ...
        function benchmarkIsIdenticalWithAndWithoutReferences(testCase)
            ...
            % (see fixtures/reference/refinv_load.m), so names, roots and
            ...
            old_switch = getenv('OPTIPROFILER_TEST_REFINV_REFERENCE');
            ...
            testCase.addTeardown(@() setenv('OPTIPROFILER_TEST_REFINV_REFERENCE', old_switch));
            cd(root);
            ...
            registerProblemLibrary(struct('name', 'refinv', 'root', fixtures, ...
                'select_function', 'refinv_select', 'load_function', 'refinv_load'));

            % The provider really serves both variants.
            library = resolveProblemLibrary('refinv');
            setenv('OPTIPROFILER_TEST_REFINV_REFERENCE', '1');
            ...
            setenv('OPTIPROFILER_TEST_REFINV_REFERENCE', '0');
            ...
            for k = 1:numel(features)
                ...
                archives = cell(1, 2);
                for variant = 1:2
                    setenv('OPTIPROFILER_TEST_REFINV_REFERENCE', num2str(2 - variant));  % '1' then '0'
                    savepath_variant = fullfile(root, sprintf('f%d-v%d', k, variant));
                    mkdir(savepath_variant);
                    options = struct('plibs', {{'refinv'}}, 'ptype', 'un', 'mindim', 1, 'maxdim', 2, ...
        ...
        function saveAndLoadRoundTrip(testCase)
            ...
            problem = TestProblemReference.constrainedProblem(record); %#ok<NASGU>
            featured = FeaturedProblem(TestProblemReference.constrainedProblem(record), Feature('noisy+permuted'), 10, 3); %#ok<NASGU>
            file = [tempname, '.mat'];
        ...
        function supersededLayoutFileLoadsAsUnknown(testCase)
            ...
            testCase.verifyEqual(loaded.problem.name, 'LEGACYREF');
            testCase.verifyEqual(loaded.problem.fun([1; 2]), 0);
        ...
        function rejectedStoredRecordsReadAsUnknown(testCase)
            ...
            valid = TestProblemReference.record();
            stored = { ...
                setfield(valid, 'mapping', 'feasible_objective/2'), ...   % a token of a later registry
                setfield(valid, 'mapping', 'penalized_merit/1'), ...
                setfield(valid, 'merit', NaN), setfield(valid, 'merit', Inf), ...
                setfield(valid, 'kind', 'certified_bound'), setfield(valid, 'source', ''), ...
                struct('fun', 0, 'maxcv', 0, 'kind', 'optimum', 'source', 'author', 'point', [1; 2]), ...
                setfield(valid, 'point', [1; 2]), rmfield(valid, 'mapping'), 0, 'optimum', {valid}}; %#ok<SFLD>
        ...
        function providerLoadPreservesTheReferenceWithoutExecutingAnything(testCase)
            ...
            writeText(fullfile(root, 'reftoy_select.m'), sprintf([ ...
                'function names = reftoy_select(~)\n', ...
                '    names = {''REF'', ''PLAIN'', ''NAN'', ''LEGACY'', ''FUTURE'', ''CALLBACK'', ''NAKED''};\n', ...
                'end\n']));
            writeText(fullfile(root, 'reftoy_never.m'), sprintf([ ...
                'function f = reftoy_never(~)\n', ...
                '    error(''TestProblemReference:evaluated'', ''The objective was evaluated while loading.'');\n', ...
                'end\n']));
            writeText(fullfile(root, 'reftoy_load.m'), sprintf([ ...
                'function problem = reftoy_load(name)\n', ...
                '    s = struct(''fun'', @reftoy_never, ''x0'', [0; 0], ''name'', name);\n', ...
                '    good = struct(''merit'', 0, ''kind'', ''optimum'', ''source'', ''catalog:reftoy'', ''mapping'', ''feasible_objective/1'');\n', ...
                '    switch name\n', ...
                '        case ''REF''\n', ...
                '            s.reference = good;\n', ...
                '        case ''NAN''\n', ...
                '            s.reference = setfield(good, ''merit'', NaN);\n', ...
                '        case ''LEGACY''\n', ...
                '            s.reference = struct(''fun'', 0, ''kind'', ''optimum'', ''source'', ''catalog:reftoy'', ''point'', [1; 2]);\n', ...
                '        case ''FUTURE''\n', ...
                '            s.reference = setfield(good, ''mapping'', ''feasible_objective/2'');\n', ...
                '        case ''CALLBACK''\n', ...
                '            s.reference = setfield(good, ''mapping'', @reftoy_never);\n', ...
                '        case ''NAKED''\n', ...
                '            s.reference = 0;\n', ...
                '    end\n', ...
                '    problem = Problem(s);\n', ...
                'end\n']));
            registerProblemLibrary(struct('name', 'reftoy', 'root', root, ...
                'select_function', 'reftoy_select', 'load_function', 'reftoy_load'));
            library = resolveProblemLibrary('reftoy');
            ...
            testCase.verifyEqual(problem.reference, struct('merit', 0, 'kind', 'optimum', ...
                'source', 'catalog:reftoy', 'mapping', 'feasible_objective/1'));  % provenance survives the load
