function varargout = LUKVLE14(action,varargin)
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        if(nargs<1)
            v_('N') = 20;  %  SIF file default value
        else
            v_('N') = varargin{1};
        end
% IE N                   99998          $-PARAMETER
        ...
        for K=v_('1'):v_('2'):v_('NC')
            ...
            pbm.gconst(ig_(['C',int2str(round(v_('K+1')))])) = 6.0;
        end
        %%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        for I=v_('1'):v_('3'):v_('N')
            pb.x0(ix_(['X',int2str(I)]),1) = 10.0;
        end
        for I=v_('2'):v_('3'):v_('N')
            pb.x0(ix_(['X',int2str(I)]),1) = 7.0;
        end
