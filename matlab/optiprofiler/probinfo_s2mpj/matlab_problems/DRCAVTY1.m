function varargout = DRCAVTY1(action,varargin)
...
%    It differs from the problems DRCAVTY2 and DRCAVTY3 by the value 
...
% IE M                   100            $-PARAMETER  n = 10000
        if(nargs<2)
            v_('RE') = 500.0;  %  SIF file default value
        else
            v_('RE') = varargin{2};
        end
        v_('M+2') = 2+v_('M');
        ...
        for I=v_('1'):v_('M')
            ...
            for J=v_('1'):v_('M')
                ...
                valA(end+1) = 20.0;
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['Y',int2str(round(v_('I-1'))),',',int2str(J)]);
                valA(end+1) = -8.0;
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['Y',int2str(round(v_('I+1'))),',',int2str(J)]);
                valA(end+1) = -8.0;
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['Y',int2str(I),',',int2str(round(v_('J-1')))]);
                valA(end+1) = -8.0;
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['Y',int2str(I),',',int2str(round(v_('J+1')))]);
                valA(end+1) = -8.0;
                irA(end+1)  = ig;
                icA(end+1)  =...
                      ix_(['Y',int2str(round(v_('I-1'))),',',int2str(round(v_('J+1')))]);
                valA(end+1) = 2.0;
                irA(end+1)  = ig;
                icA(end+1)  =...
                      ix_(['Y',int2str(round(v_('I+1'))),',',int2str(round(v_('J-1')))]);
                ...
                valA(end+1) = 2.0;
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['Y',int2str(round(v_('I-2'))),',',int2str(J)]);
                valA(end+1) = 1.0;
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['Y',int2str(round(v_('I+2'))),',',int2str(J)]);
                valA(end+1) = 1.0;
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['Y',int2str(I),',',int2str(round(v_('J-2')))]);
                valA(end+1) = 1.0;
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['Y',int2str(I),',',int2str(round(v_('J+2')))]);
                valA(end+1) = 1.0;
