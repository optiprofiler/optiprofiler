function varargout = HS31(action,varargin)
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        pb.xlower(ix_('X1'),1) = -10.0;
        pb.xlower(ix_('X2'),1) = 1.0;
        pb.xlower(ix_('X3'),1) = -10.0;
        pb.xupper(ix_('X1')) = 10.0;
        pb.xupper(ix_('X2')) = 10.0;
        ...
        ig = ig_('CONSTR');
        posel = length(pbm.grelt{ig})+1;
        pbm.grelt{ig}(posel) = ie_('CONSTR1');
        nlc = union(nlc,ig);
        pbm.grelw{ig}(posel) = 1.;
        %%%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
%   Solution
