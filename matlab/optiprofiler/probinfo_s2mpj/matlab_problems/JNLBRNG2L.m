function varargout = JNLBRNG2L(action,varargin)
...
%    default zero lower bound added, Apr 2024
% 
...
% IE PY                  125            $-PARAMETER
        ...
        for I=v_('1'):v_('PT-1')
            for J=v_('1'):v_('PY-1')
                [ig,ig_] = s2xlib('ii',['GR',int2str(I),',',int2str(J)],ig_);
                gtype{ig} = '<>';
                pbm.gscale(ig,1) = 2.0;
            end
        end
        for I=v_('2'):v_('PT')
            for J=v_('2'):v_('PY')
                [ig,ig_] = s2xlib('ii',['GL',int2str(I),',',int2str(J)],ig_);
                gtype{ig} = '<>';
                pbm.gscale(ig,1) = 2.0;
            end
        end
