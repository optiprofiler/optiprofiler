function varargout = PRODPL1(action,varargin)
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        pbm.grelt{ig}(posel) = ie_('NLE1');
        nlc = union(nlc,ig);
        pbm.grelw{ig}(posel) = 1.;
        ig = ig_('SMOOTH2');
        posel = length(pbm.grelt{ig})+1;
        pbm.grelt{ig}(posel) = ie_('NLE2');
        nlc = union(nlc,ig);
        pbm.grelw{ig}(posel) = 1.;
        ig = ig_('SMOOTH3');
        posel = length(pbm.grelt{ig})+1;
        pbm.grelt{ig}(posel) = ie_('NLE3');
        nlc = union(nlc,ig);
        pbm.grelw{ig}(posel) = 1.;
        ig = ig_('SMOOTH4');
        posel = length(pbm.grelt{ig})+1;
        pbm.grelt{ig}(posel) = ie_('NLE4');
        nlc = union(nlc,ig);
        pbm.grelw{ig}(posel) = 1.;
        %%%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
%    Solution
...
% **********************
    ...
    case 'eSQMRSQ'
        ...
        IV_(2) = U_(2,:)*EV_;
        U1MU2 = IV_(1)-IV_(2);
        varargout{1} = U1MU2^2-0.1*IV_(2)^2;
        if(nargout>1)
            g_(1,1) = 2.0*U1MU2;
            g_(2,1) = -2.0*U1MU2-2.0*0.1*IV_(2);
            varargout{2} = U_.'*g_;
            if(nargout>2)
                H_ = sparse(2,2);
                H_(1,1) = 2.0;
