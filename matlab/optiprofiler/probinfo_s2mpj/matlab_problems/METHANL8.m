function varargout = METHANL8(action,varargin)
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        if(isKey(ix_,'T0'))
            ...
            pb.y0(find(pbm.congrps==ig_('T0')),1) = 120.0;
        end
        if(isKey(ix_,'T1'))
            pb.x0(ix_('T1'),1) = 110.0;
        else
            pb.y0(find(pbm.congrps==ig_('T1')),1) = 110.0;
        end
        if(isKey(ix_,'T2'))
            pb.x0(ix_('T2'),1) = 100.0;
        else
            pb.y0(find(pbm.congrps==ig_('T2')),1) = 100.0;
        end
        if(isKey(ix_,'T3'))
            pb.x0(ix_('T3'),1) = 88.0;
        else
            pb.y0(find(pbm.congrps==ig_('T3')),1) = 88.0;
        end
        if(isKey(ix_,'T4'))
            pb.x0(ix_('T4'),1) = 86.0;
        else
            pb.y0(find(pbm.congrps==ig_('T4')),1) = 86.0;
        end
        if(isKey(ix_,'T5'))
            pb.x0(ix_('T5'),1) = 84.0;
        else
            pb.y0(find(pbm.congrps==ig_('T5')),1) = 84.0;
        end
        if(isKey(ix_,'T6'))
            pb.x0(ix_('T6'),1) = 80.0;
        else
            pb.y0(find(pbm.congrps==ig_('T6')),1) = 80.0;
        end
        if(isKey(ix_,'T7'))
            pb.x0(ix_('T7'),1) = 76.0;
        else
            pb.y0(find(pbm.congrps==ig_('T7')),1) = 76.0;
        end
        if(isKey(ix_,'V0'))
