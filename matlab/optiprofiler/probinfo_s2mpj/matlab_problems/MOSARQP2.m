function varargout = MOSARQP2(action,varargin)
...
% RE COND                3.0            $-PARAMETER
        ...
        for I=v_('1'):v_('3'):v_('N-2')
            v_('I+1') = 1+I;
            v_('I+2') = 2+I;
            v_(['XC',int2str(I)]) = -1.0;
            v_(['XC',int2str(round(v_('I+1')))]) = -1.0;
            v_(['XC',int2str(round(v_('I+2')))]) = 1.0;
        end
        v_(['XC',int2str(round(v_('N-1')))]) = -1.0;
        ...
        cnames{ig} = ['CS',int2str(round(v_('1')))];
        irA(end+1)  = ig;
        icA(end+1)  = ix_(['X',int2str(round(v_('RTN+1')))]);
        valA(end+1) = -1.0;
        irA(end+1)  = ig;
        ...
        for I=v_('2'):v_('RTN-1')
            ...
            valA(end+1) = 4.0;
            irA(end+1)  = ig;
            icA(end+1)  = ix_(['X',int2str(round(v_('I+RTN')))]);
            valA(end+1) = -1.0;
            irA(end+1)  = ig;
        ...
        irA(end+1)  = ig;
        icA(end+1)  = ix_(['X',int2str(round(v_('RTN')))]);
        valA(end+1) = 4.0;
        [ig,ig_] = s2mpjlib('ii',['CS',int2str(round(v_('RTN')))],ig_);
        gtype{ig}  = '>=';
        cnames{ig} = ['CS',int2str(round(v_('RTN')))];
        ...
        valA(end+1) = -1.0;
        irA(end+1)  = ig;
        icA(end+1)  = ix_(['X',int2str(round(v_('2RTN')))]);
        valA(end+1) = -1.0;
        v_('JS') = v_('RTN');
        for J=v_('RTN+1'):v_('RTN'):v_('M-RTN+1')
            ...
            valA(end+1) = -1.0;
            irA(end+1)  = ig;
            icA(end+1)  = ix_(['X',int2str(round(v_('J-RTN')))]);
            valA(end+1) = -1.0;
            irA(end+1)  = ig;
            icA(end+1)  = ix_(['X',int2str(round(v_('J+RTN')))]);
            valA(end+1) = -1.0;
            ...
            irA(end+1)  = ig;
            icA(end+1)  = ix_(['X',int2str(round(v_('JS')))]);
            valA(end+1) = 4.0;
            irA(end+1)  = ig;
            icA(end+1)  = ix_(['X',int2str(round(v_('JS-1')))]);
            valA(end+1) = -1.0;
            [ig,ig_] = s2mpjlib('ii',['CS',int2str(round(v_('JS')))],ig_);
            gtype{ig}  = '>=';
            cnames{ig} = ['CS',int2str(round(v_('JS')))];
            irA(end+1)  = ig;
            icA(end+1)  = ix_(['X',int2str(round(v_('JS-RTN')))]);
            valA(end+1) = -1.0;
            irA(end+1)  = ig;
            icA(end+1)  = ix_(['X',int2str(round(v_('JS+RTN')))]);
            valA(end+1) = -1.0;
        ...
        for I=v_('K'):v_('M'):v_('M')
            ...
            cnames{ig} = ['CS',int2str(round(v_('K')))];
            irA(end+1)  = ig;
            icA(end+1)  = ix_(['X',int2str(round(v_('K-RTN')))]);
            valA(end+1) = -1.0;
            irA(end+1)  = ig;
            icA(end+1)  = ix_(['X',int2str(round(v_('K+RTN')))]);
            valA(end+1) = -1.0;
