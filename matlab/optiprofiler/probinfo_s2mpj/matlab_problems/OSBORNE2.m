function varargout = OSBORNE2(action,varargin)
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        if(isKey(ix_,'X1'))
            ...
            pb.y0(find(pbm.congrps==ig_('X1')),1) = 1.3;
        end
        if(isKey(ix_,'X2'))
            pb.x0(ix_('X2'),1) = 0.65;
        else
            pb.y0(find(pbm.congrps==ig_('X2')),1) = 0.65;
        end
        if(isKey(ix_,'X3'))
            pb.x0(ix_('X3'),1) = 0.65;
        else
            pb.y0(find(pbm.congrps==ig_('X3')),1) = 0.65;
        end
        if(isKey(ix_,'X4'))
            pb.x0(ix_('X4'),1) = 0.7;
        else
            pb.y0(find(pbm.congrps==ig_('X4')),1) = 0.7;
        end
        if(isKey(ix_,'X5'))
            pb.x0(ix_('X5'),1) = 0.6;
        else
            pb.y0(find(pbm.congrps==ig_('X5')),1) = 0.6;
        end
        if(isKey(ix_,'X6'))
            pb.x0(ix_('X6'),1) = 3.0;
        else
            pb.y0(find(pbm.congrps==ig_('X6')),1) = 3.0;
        end
        if(isKey(ix_,'X7'))
            pb.x0(ix_('X7'),1) = 5.0;
        else
            pb.y0(find(pbm.congrps==ig_('X7')),1) = 5.0;
        end
        if(isKey(ix_,'X8'))
            pb.x0(ix_('X8'),1) = 7.0;
        else
            pb.y0(find(pbm.congrps==ig_('X8')),1) = 7.0;
        end
        if(isKey(ix_,'X9'))
            pb.x0(ix_('X9'),1) = 2.0;
        else
            pb.y0(find(pbm.congrps==ig_('X9')),1) = 2.0;
        end
        if(isKey(ix_,'X10'))
            pb.x0(ix_('X10'),1) = 4.5;
        else
            pb.y0(find(pbm.congrps==ig_('X10')),1) = 4.5;
        end
        if(isKey(ix_,'X11'))
            pb.x0(ix_('X11'),1) = 5.5;
        else
            pb.y0(find(pbm.congrps==ig_('X11')),1) = 5.5;
        end
        %%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
