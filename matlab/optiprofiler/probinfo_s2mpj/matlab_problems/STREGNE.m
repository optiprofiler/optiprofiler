function varargout = STREGNE(action,varargin)
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        if(isKey(ix_,'X1'))
            ...
            pb.y0(find(pbm.congrps==ig_('X1')),1) = -1.2;
        end
        if(isKey(ix_,'X2'))
            pb.x0(ix_('X2'),1) = 1.0;
        else
        ...
        if(isKey(ix_,'X3'))
            pb.x0(ix_('X3'),1) = 1.0e+10;
        else
            pb.y0(find(pbm.congrps==ig_('X3')),1) = 1.0e+10;
        end
        if(isKey(ix_,'X4'))
            pb.x0(ix_('X4'),1) = 1.0e+10;
        else
            pb.y0(find(pbm.congrps==ig_('X4')),1) = 1.0e+10;
        end
        %%%%%%%%%%%%%%%%%%%%% QUADRATIC %%%%%%%%%%%%%%%%%%%
