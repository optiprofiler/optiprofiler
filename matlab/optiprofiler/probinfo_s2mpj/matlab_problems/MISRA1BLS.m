function varargout = MISRA1BLS(action,varargin)
...
% **********************
    ...
    case 'eE3'
        ...
        EV_  = varargin{1};
        iel_ = varargin{2};
        R = 1.0+0.5*EV_(2)*pbm.elpar{iel_}(1);
        R2 = R*R;
        R3 = R*R2;
        varargout{1} = -EV_(1)/R2;
        if(nargout>1)
            g_(1,1) = -1.0/R2;
            g_(2,1) = EV_(1)*pbm.elpar{iel_}(1)/R3;
            varargout{2} = g_;
            if(nargout>2)
                H_ = sparse(2,2);
                H_(1,2) = pbm.elpar{iel_}(1)/R3;
                H_(2,1) = H_(1,2);
                H_(2,2) = -1.5*EV_(1)*pbm.elpar{iel_}(1)^2/R2^2;
                varargout{3} = H_;
            end
        end
