function varargout = HS51(action,varargin)
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        if(isKey(ix_,'X1'))
            ...
            pb.y0(find(pbm.congrps==ig_('X1')),1) = 2.5;
        end
        if(isKey(ix_,'X2'))
            pb.x0(ix_('X2'),1) = 0.5;
        else
        ...
        elftv{it}{1} = 'V1';
        elftv{it}{2} = 'V2';
        [it,iet_] = s2mpjlib( 'ii', 'eSQ1P2',iet_);
        elftv{it}{1} = 'V1';
        elftv{it}{2} = 'V2';
        ...
        ename = 'E2';
        [ie,ie_] = s2mpjlib('ii',ename,ie_);
        pbm.elftype{ie} = 'eSQ1P2';
        ielftype(ie) = iet_('eSQ1P2');
        vname = 'X2';
        [iv,ix_,pb] = s2mpjlib('nlx',vname,ix_,pb,1,[],[],[]);
        posev = find(strcmp('V1',elftv{ielftype(ie)}));
        pbm.elvar{ie}(posev) = iv;
...
% **********************
    ...
    case 'eSQ1P2'
        ...
        U_(1,2) = U_(1,2)+1;
        IV_(1) = U_(1,:)*EV_;
        varargout{1} = (IV_(1)-2.0)^2;
        if(nargout>1)
            g_(1,1) = 2.0*(IV_(1)-2.0);
            varargout{2} = U_.'*g_;
            if(nargout>2)
                H_(1,1) = 2.0;
                varargout{3} = U_.'*H_*U_;
