...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        iv = ix_('QF');
        if(size(pbm.A,1)>=ig&&size(pbm.A,2)>=iv)
            pbm.A(ig,iv) = v_('1/790')+pbm.A(ig,iv);
        else
            pbm.A(ig,iv) = v_('1/790');
        end
