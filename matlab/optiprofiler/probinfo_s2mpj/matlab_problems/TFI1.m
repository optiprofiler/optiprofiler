function varargout = TFI1(action,varargin)
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        for I=v_('0'):v_('M')
            ...
            v_('C') = v_('2S4T')-v_('E2T');
            pbm.gconst(ig_(['CG',int2str(I)])) = v_('C');
        end
        %%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        elftv{it}{1} = 'X';
        [it,iet_] = s2mpjlib( 'ii', 'eXEYT',iet_);
        elftv{it}{1} = 'X';
        elftv{it}{2} = 'Y';
        elftp{it}{1} = 'TT';
        %%%%%%%%%%%%%%%%%%% ELEMENT USES %%%%%%%%%%%%%%%%%%
        ...
        for I=v_('0'):v_('M')
            ename = ['E',int2str(I)];
            [ie,ie_] = s2mpjlib('ii',ename,ie_);
            pbm.elftype{ie} = 'eXEYT';
            ielftype(ie) = iet_('eXEYT');
            vname = 'X2';
            [iv,ix_,pb] = s2mpjlib('nlx',vname,ix_,pb,1,[],[],[]);
            posev = find(strcmp('X',elftv{ielftype(ie)}));
            pbm.elvar{ie}(posev) = iv;
            ...
            v_('RI') = I;
            v_('T') = v_('RI')*v_('H');
            [~,posep] = ismember('TT',elftp{ielftype(ie)});
...
% **********************
    ...
    case 'eXEYT'
        ...
        varargout{1} = EV_(1)*EYT;
        if(nargout>1)
            g_(1,1) = EYT;
            g_(2,1) = EV_(1)*pbm.elpar{iel_}(1)*EYT;
            varargout{2} = g_;
            if(nargout>2)
                H_ = sparse(2,2);
