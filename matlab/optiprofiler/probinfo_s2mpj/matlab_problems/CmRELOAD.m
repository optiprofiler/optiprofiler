...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        gtype{ig} = '<>';
        irA(end+1)  = ig;
        icA(end+1)  = ix_(['KEFF',int2str(round(v_('T')))]);
        valA(end+1) = -1.0;
        for K=v_('1'):v_('L')
            for J=v_('1'):v_('M')
                for I=v_('1'):v_('N')
                    ...
                    irA(end+1)  = ig;
                    icA(end+1)  = ix_(['X',int2str(I),',',int2str(K),',',int2str(J)]);
                    valA(end+1) = v_(['V',int2str(I)]);
                end
            end
        end
        for I=v_('1'):v_('N')
        ...
        for I=v_('1'):v_('N')
            ...
            irA(end+1)  = ig;
            icA(end+1)  = ix_(['KINF',int2str(I),',',int2str(round(v_('1')))]);
            valA(end+1) = -1.0;
            for J=v_('1'):v_('M')
                [ig,ig_] = s2mpjlib('ii',['PLAC',int2str(I)],ig_);
                gtype{ig}  = '==';
                ...
                irA(end+1)  = ig;
                icA(end+1)  =...
                      ix_(['X',int2str(I),',',int2str(round(v_('1'))),',',int2str(J)]);
                valA(end+1) = v_('KFRESH');
            end
        end
        for I=v_('1'):v_('N')
            for S=v_('1'):v_('T')
        ...
        for I=v_('1'):v_('N')
            for S=v_('1'):v_('T-1')
                ...
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['KINF',int2str(I),',',int2str(round(v_('R')))]);
                valA(end+1) = -1.0;
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['KINF',int2str(I),',',int2str(S)]);
                valA(end+1) = 1.0;
            end
        end
        for S=v_('1'):v_('T')
