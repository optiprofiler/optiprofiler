function varargout = FMINSRF2(action,varargin)
...
% IE P                   125            $-PARAMETER n = 15625
        ...
        [ig,ig_] = s2mpjlib('ii','MID',ig_);
        gtype{ig} = '<>';
        iv = ix_(['X',int2str(round(v_('MID'))),',',int2str(round(v_('MID')))]);
        if(size(pbm.A,1)>=ig&&size(pbm.A,2)>=iv)
            pbm.A(ig,iv) = 1.0+pbm.A(ig,iv);
        else
            pbm.A(ig,iv) = 1.0;
        end
        pbm.gscale(ig,1) = v_('P2');
        %%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        pb.n   = numEntries(ix_);
        ngrp   = numEntries(ig_);
        pbm.objgrps = [1:ngrp];
