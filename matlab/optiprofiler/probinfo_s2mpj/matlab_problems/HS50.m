function varargout = HS50(action,varargin)
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        if(isKey(ix_,'X1'))
            ...
            pb.y0(find(pbm.congrps==ig_('X1')),1) = 35.0;
        end
        if(isKey(ix_,'X2'))
            pb.x0(ix_('X2'),1) = -31.0;
        else
            pb.y0(find(pbm.congrps==ig_('X2')),1) = -31.0;
        end
        if(isKey(ix_,'X3'))
            pb.x0(ix_('X3'),1) = 11.0;
        else
            pb.y0(find(pbm.congrps==ig_('X3')),1) = 11.0;
        end
        if(isKey(ix_,'X4'))
            pb.x0(ix_('X4'),1) = 5.0;
        else
