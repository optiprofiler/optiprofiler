...
% IE N                   99             $-PARAMETER     twod_0.mod value
        ...
        for I=v_('1'):v_('N1')
            ...
            for J=v_('1'):v_('N1')
                ...
                for K=v_('0'):v_('M1')
                    ...
                    irA(end+1)  = ig;
                    icA(end+1)  =...
                          ix_(['Y',int2str(round(v_('K+'))),',',int2str(I),',',int2str(J)]);
                    valA(end+1) = v_('1/DT+2/H2');
                    irA(end+1)  = ig;
                    icA(end+1)  = ix_(['Y',int2str(K),',',int2str(I),',',int2str(J)]);
                    valA(end+1) = v_('-1/DT+2/H2');
                    irA(end+1)  = ig;
                    icA(end+1)  =...
                          ix_(['Y',int2str(K),',',int2str(I),',',int2str(round(v_('J-')))]);
                    valA(end+1) = v_('-.1/2H2');
                    irA(end+1)  = ig;
                    icA(end+1)  =...
                          ix_(['Y',int2str(K),',',int2str(I),',',int2str(round(v_('J+')))]);
                    ...
                    irA(end+1)  = ig;
                    icA(end+1)  =...
                          ix_(['Y',int2str(K),',',int2str(round(v_('I-'))),',',int2str(J)]);
                    valA(end+1) = v_('-.1/2H2');
                    irA(end+1)  = ig;
                    icA(end+1)  =...
                          ix_(['Y',int2str(K),',',int2str(round(v_('I+'))),',',int2str(J)]);
        ...
        for I=v_('1'):v_('N1')
            for K=v_('1'):v_('M')
                ...
                irA(end+1)  = ig;
                icA(end+1)  =...
                      ix_(['Y',int2str(K),',',int2str(I),',',int2str(round(v_('N2')))]);
                valA(end+1) = v_('1/2DY');
                irA(end+1)  = ig;
                icA(end+1)  =...
                      ix_(['Y',int2str(K),',',int2str(I),',',int2str(round(v_('N1')))]);
                valA(end+1) = v_('-2/DY');
                irA(end+1)  = ig;
                icA(end+1)  =...
                      ix_(['Y',int2str(K),',',int2str(I),',',int2str(round(v_('N')))]);
                valA(end+1) = v_('3/2DY+1');
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['U',int2str(K),',',int2str(I)]);
                valA(end+1) = -1.0;
                ...
                irA(end+1)  = ig;
                icA(end+1)  =...
                      ix_(['Y',int2str(K),',',int2str(I),',',int2str(round(v_('2')))]);
                valA(end+1) = v_('1/2DY');
                irA(end+1)  = ig;
                icA(end+1)  =...
                      ix_(['Y',int2str(K),',',int2str(I),',',int2str(round(v_('1')))]);
                valA(end+1) = v_('-2/DY');
                irA(end+1)  = ig;
                icA(end+1)  =...
                      ix_(['Y',int2str(K),',',int2str(I),',',int2str(round(v_('0')))]);
                valA(end+1) = v_('3/2DY+1');
            end
        end
        for J=v_('1'):v_('N1')
            for K=v_('1'):v_('M')
                ...
                irA(end+1)  = ig;
                icA(end+1)  =...
                      ix_(['Y',int2str(K),',',int2str(round(v_('2'))),',',int2str(J)]);
                valA(end+1) = v_('1/2DX');
                irA(end+1)  = ig;
                icA(end+1)  =...
                      ix_(['Y',int2str(K),',',int2str(round(v_('1'))),',',int2str(J)]);
                valA(end+1) = v_('-2/DX');
                irA(end+1)  = ig;
                icA(end+1)  =...
                      ix_(['Y',int2str(K),',',int2str(round(v_('0'))),',',int2str(J)]);
                valA(end+1) = v_('3/2DX+1');
                [ig,ig_] = s2mpjlib('ii',['B4',int2str(K),',',int2str(J)],ig_);
                gtype{ig}  = '==';
                cnames{ig} = ['B4',int2str(K),',',int2str(J)];
                irA(end+1)  = ig;
                icA(end+1)  =...
                      ix_(['Y',int2str(K),',',int2str(round(v_('N2'))),',',int2str(J)]);
                valA(end+1) = v_('1/2DX');
                irA(end+1)  = ig;
                icA(end+1)  =...
                      ix_(['Y',int2str(K),',',int2str(round(v_('N1'))),',',int2str(J)]);
                valA(end+1) = v_('-2/DX');
                irA(end+1)  = ig;
                icA(end+1)  =...
                      ix_(['Y',int2str(K),',',int2str(round(v_('N'))),',',int2str(J)]);
                valA(end+1) = v_('3/2DX+1');
            end
        end
        %%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
