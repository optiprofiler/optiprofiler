function varargout = CB3(action,varargin)
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        ig = ig_('C2');
        posel = length(pbm.grelt{ig})+1;
        pbm.grelt{ig}(posel) = ie_('SSQ1');
        nlc = union(nlc,ig);
        pbm.grelw{ig}(posel) = - 1.0;
        posel = posel+1;
        pbm.grelt{ig}(posel) = ie_('SSQ2');
        pbm.grelw{ig}(posel) = - 1.0;
        ig = ig_('C3');
        posel = length(pbm.grelt{ig})+1;
        pbm.grelt{ig}(posel) = ie_('EEX');
        nlc = union(nlc,ig);
