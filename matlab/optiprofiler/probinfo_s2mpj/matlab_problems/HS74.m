function varargout = HS74(action,varargin)
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        v_('-A') = -1.0*v_('A');
        pbm.gconst(ig_('C1')) = v_('-A');
        pbm.gconst(ig_('C2')) = v_('-A');
        pbm.gconst(ig_('C3')) = -894.8;
        pbm.gconst(ig_('C4')) = -894.8;
        pbm.gconst(ig_('C5')) = -1294.8;
        %%%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        pb.xlower = zeros(pb.n,1);
        pb.xupper = Inf*ones(pb.n,1);
        pb.xupper(ix_('X1')) = 1200.0;
        pb.xupper(ix_('X2')) = 1200.0;
        pb.xlower(ix_('X3'),1) = v_('-A');
        pb.xupper(ix_('X3')) = v_('A');
        pb.xlower(ix_('X4'),1) = v_('-A');
