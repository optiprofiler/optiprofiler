function varargout = HS34(action,varargin)
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        if(isKey(ix_,'X2'))
            ...
            pb.y0(find(pbm.congrps==ig_('X2')),1) = 1.05;
        end
        if(isKey(ix_,'X3'))
            pb.x0(ix_('X3'),1) = 2.9;
        else
            pb.y0(find(pbm.congrps==ig_('X3')),1) = 2.9;
        end
        %%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        ...
        vname = 'X1';
        [iv,ix_,pb] = s2mpjlib('nlx',vname,ix_,pb,1,[],100.0,[]);
