function varargout = HS11(action,varargin)
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        if(isKey(ix_,'X1'))
            ...
            pb.y0(find(pbm.congrps==ig_('X1')),1) = 4.9;
        end
        if(isKey(ix_,'X2'))
            pb.x0(ix_('X2'),1) = 0.1;
        else
        ...
        [~,posep] = ismember('SH',elftp{ielftype(ie)});
