function varargout = ENSO(action,varargin)
...
% 
%    classification = 'C-CNOR2-MN-9-168'
% 
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        if(isKey(ix_,'B1'))
            ...
            pb.y0(find(pbm.congrps==ig_('B1')),1) = 11.0;
        end
        if(isKey(ix_,'B2'))
            pb.x0(ix_('B2'),1) = 3.0;
        else
            pb.y0(find(pbm.congrps==ig_('B2')),1) = 3.0;
        end
        if(isKey(ix_,'B3'))
            pb.x0(ix_('B3'),1) = 0.5;
        else
            pb.y0(find(pbm.congrps==ig_('B3')),1) = 0.5;
        end
        if(isKey(ix_,'B4'))
            pb.x0(ix_('B4'),1) = 40.0;
        else
            pb.y0(find(pbm.congrps==ig_('B4')),1) = 40.0;
        end
        if(isKey(ix_,'B5'))
            pb.x0(ix_('B5'),1) = -0.7;
        else
            pb.y0(find(pbm.congrps==ig_('B5')),1) = -0.7;
        end
        if(isKey(ix_,'B6'))
            pb.x0(ix_('B6'),1) = -1.3;
        else
            pb.y0(find(pbm.congrps==ig_('B6')),1) = -1.3;
        end
        if(isKey(ix_,'B7'))
            pb.x0(ix_('B7'),1) = 25.0;
        else
            pb.y0(find(pbm.congrps==ig_('B7')),1) = 25.0;
        end
        if(isKey(ix_,'B8'))
            pb.x0(ix_('B8'),1) = -0.3;
        else
            pb.y0(find(pbm.congrps==ig_('B8')),1) = -0.3;
        end
        if(isKey(ix_,'B9'))
            pb.x0(ix_('B9'),1) = 1.4;
        else
            pb.y0(find(pbm.congrps==ig_('B9')),1) = 1.4;
        end
        %%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
