...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        for I=v_('1'):v_('N-P')
            for J=v_('1'):v_('P')
                ...
                gtype{ig} = '<>';
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['X',int2str(round(v_('I+J-1')))]);
                valA(end+1) = v_('CL');
            end
        end
        ...
        for I=v_('1'):v_('NY')
            ...
            [ie,ie_] = s2mpjlib('ii',ename,ie_);
            pbm.elftype{ie} = 'en3P';
            ielftype(ie) = iet_('en3P');
            vname = ['X',int2str(I)];
            [iv,ix_,pb] = s2mpjlib('nlx',vname,ix_,pb,1,[],[],[]);
            posev = find(strcmp('X',elftv{ielftype(ie)}));
            pbm.elvar{ie}(posev) = iv;
            v_('NY+I') = v_('NY')+I;
            vname = ['X',int2str(round(v_('NY+I')))];
            [iv,ix_,pb] = s2mpjlib('nlx',vname,ix_,pb,1,[],[],[]);
    ...
    case 'en3P'
        ...
        EV_  = varargin{1};
        iel_ = varargin{2};
        varargout{1} = EV_(1)*EV_(2)*EV_(3)+2.0*EV_(3)*EV_(3);
        if(nargout>1)
            g_(1,1) = EV_(2)*EV_(3);
            g_(2,1) = EV_(1)*EV_(3);
            g_(3,1) = EV_(1)*EV_(2)+4.0*EV_(3);
            varargout{2} = g_;
            if(nargout>2)
                H_ = sparse(3,3);
                H_(1,2) = EV_(3);
