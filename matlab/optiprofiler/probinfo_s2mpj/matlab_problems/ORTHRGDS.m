function varargout = ORTHRGDS(action,varargin)
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        if(nargs<1)
            v_('NPTS') = 20;  %  SIF file default value
        else
            v_('NPTS') = varargin{1};
        end
% IE NPTS                5000           $-PARAMETER n = 10003
        ...
        for I=v_('TDPulo'):v_('TDPuhi')
            if(isKey(ix_,['X',int2str(I)]))
                pb.x0(ix_(['X',int2str(I)]),1) = 1.8;
            else
                pb.y0(find(pbm.congrps==ig_(['X',int2str(I)])),1) = 1.8;
            end
            if(isKey(ix_,['Y',int2str(I)]))
                pb.x0(ix_(['Y',int2str(I)]),1) = 1.0;
            else
                pb.y0(find(pbm.congrps==ig_(['Y',int2str(I)])),1) = 1.0;
            end
        end
        %%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
