function varargout = DTOC1ND(action,varargin)
...
%    The nonlinearity parameter mu is set to 1.0.
% 
...
% IE NY                  10             $-PARAMETER # states   }
        if(nargs<4)
            v_('MU') = 1.0;  %  SIF file default value
        else
            v_('MU') = varargin{4};
        end
        v_('N-1') = -1+v_('N');
        ...
        for T=v_('1'):v_('N-1')
            for I=v_('1'):v_('NX')
                ...
                gtype{ig} = '<>';
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['X',int2str(T),',',int2str(I)]);
                valA(end+1) = 1.0;
            end
        end
        for T=v_('1'):v_('N')
            for I=v_('1'):v_('NY')
                ...
                gtype{ig} = '<>';
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['Y',int2str(T),',',int2str(I)]);
                valA(end+1) = 1.0;
            end
        end
        for T=v_('1'):v_('N-1')
            ...
            for I=v_('1'):v_('NX')
                ...
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['X',int2str(T),',',int2str(I)]);
                valA(end+1) = v_(['B',int2str(round(v_('1'))),',',int2str(I)]);
            end
            for J=v_('2'):v_('NY-1')
                v_('J-1') = -1+J;
                v_('J+1') = 1+J;
                ...
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['Y',int2str(round(v_('T+1'))),',',int2str(J)]);
                valA(end+1) = -1.0;
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['Y',int2str(T),',',int2str(J)]);
                valA(end+1) = 0.5;
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['Y',int2str(T),',',int2str(round(v_('J-1')))]);
                valA(end+1) = -0.25;
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['Y',int2str(T),',',int2str(round(v_('J+1')))]);
                valA(end+1) = 0.25;
                for I=v_('1'):v_('NX')
                    ...
                    irA(end+1)  = ig;
                    icA(end+1)  = ix_(['X',int2str(T),',',int2str(I)]);
                    valA(end+1) = v_(['B',int2str(J),',',int2str(I)]);
                end
            end
            [ig,ig_] =...
            ...
            irA(end+1)  = ig;
            icA(end+1)  =...
                  ix_(['Y',int2str(round(v_('T+1'))),',',int2str(round(v_('NY')))]);
            valA(end+1) = -1.0;
            [ig,ig_] =...
                  s2mpjlib('ii',['TT',int2str(T),',',int2str(round(v_('NY')))],ig_);
            gtype{ig}  = '==';
            cnames{ig} = ['TT',int2str(T),',',int2str(round(v_('NY')))];
            irA(end+1)  = ig;
            icA(end+1)  = ix_(['Y',int2str(T),',',int2str(round(v_('NY')))]);
            valA(end+1) = 0.5;
            irA(end+1)  = ig;
            icA(end+1)  = ix_(['Y',int2str(T),',',int2str(round(v_('NY-1')))]);
            valA(end+1) = -0.25;
            for I=v_('1'):v_('NX')
                [ig,ig_] =...
                      s2mpjlib('ii',['TT',int2str(T),',',int2str(round(v_('NY')))],ig_);
                ...
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['X',int2str(T),',',int2str(I)]);
                valA(end+1) = v_(['B',int2str(round(v_('NY'))),',',int2str(I)]);
            end
        end
        %%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        pb.n   = ix_.Count;
