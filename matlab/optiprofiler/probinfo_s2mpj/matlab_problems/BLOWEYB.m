function varargout = BLOWEYB(action,varargin)
...
% IE N                   8000           $-PARAMETER  n = 16002, m = 8002
        ...
        for I=v_('0'):v_('N')
            ...
            if(size(pbm.A,1)>=ig&&size(pbm.A,2)>=iv)
                pbm.A(ig,iv) = v_('VAL')+pbm.A(ig,iv);
            else
                pbm.A(ig,iv) = v_('VAL');
            end
        ...
        if(size(pbm.A,1)>=ig&&size(pbm.A,2)>=iv)
            pbm.A(ig,iv) = v_('-1/N**2')+pbm.A(ig,iv);
        else
            pbm.A(ig,iv) = v_('-1/N**2');
        end
        for I=v_('1'):v_('N-1')
            v_('I+1') = 1+I;
            v_('I-1') = -1+I;
