function varargout = DECONVC(action,varargin)
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        for I=v_('1'):v_('LGSG')
            ig = ig_('ENERG');
            posel = length(pbm.grelt{ig})+1;
            pbm.grelt{ig}(posel) = ie_(['ESQ',int2str(I)]);
            nlc = union(nlc,ig);
            pbm.grelw{ig}(posel) = 1.;
        end
