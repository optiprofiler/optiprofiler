function varargout = ROSZMAN1(action,varargin)
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        if(isKey(ix_,'B2'))
            ...
            pb.y0(find(pbm.congrps==ig_('B2')),1) = -0.00001;
        end
        if(isKey(ix_,'B3'))
            pb.x0(ix_('B3'),1) = 1000.0;
        else
            pb.y0(find(pbm.congrps==ig_('B3')),1) = 1000.0;
        end
        if(isKey(ix_,'B4'))
            pb.x0(ix_('B4'),1) = -100.0;
        else
            pb.y0(find(pbm.congrps==ig_('B4')),1) = -100.0;
        end
        %%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
