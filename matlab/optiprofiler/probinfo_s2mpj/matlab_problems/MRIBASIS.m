...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        irA(end+1)  = ig;
        icA(end+1)  =...
              ix_(['X',int2str(round(v_('2'))),',',int2str(round(v_('xm')))]);
        valA(end+1) = 1;
        for j=v_('1'):v_('2')
            for k=v_('2'):v_('xm-2')
                v_('k+') = 1+k;
                ...
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['X',int2str(j),',',int2str(round(v_('k+')))]);
                valA(end+1) = 1;
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['X',int2str(j),',',int2str(k)]);
                valA(end+1) = -1;
            end
        end
        ...
        for i=v_('1'):v_('3')
            for j=v_('1'):v_('2')
                for k=v_('1'):v_('Lm-')
                    ...
                    irA(end+1)  = ig;
                    icA(end+1)  =...
                          ix_(['L',int2str(i),',',int2str(j),',',int2str(round(v_('k+')))]);
                    valA(end+1) = 1;
                    irA(end+1)  = ig;
                    icA(end+1)  = ix_(['L',int2str(i),',',int2str(j),',',int2str(k)]);
                    valA(end+1) = -1;
                    irA(end+1)  = ig;
                    icA(end+1)  = ix_(['X',int2str(j),',',int2str(round(v_('2k')))]);
                    valA(end+1) = v_('-k1');
                    irA(end+1)  = ig;
                    icA(end+1)  = ix_(['X',int2str(j),',',int2str(round(v_('2k-')))]);
                    valA(end+1) = v_('k1');
        ...
        for i=v_('1'):v_('3')
            for k=v_('2'):v_('Lm-')
                ...
                irA(end+1)  = ig;
                icA(end+1)  =...
                      ix_(['L',int2str(i),',',int2str(round(v_('2'))),',',int2str(k)]);
                valA(end+1) = v_(['S',int2str(round(v_('3'))),',',int2str(i)]);
            end
        end
        for i=v_('1'):v_('3')
