function varargout = LEVYMONT8C(action,varargin)
...
%                     ( K*pi/n) * [ s(x1,L,C)^2 + \sum_{i=2}^n 
%     p(x_{i-1},x_i,L,C,A)^2 ]
%    where
...
switch(action)
    ...
    pbm.name     = 'LEVYMONT8C';
        ...
        %%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        pb.xlower = -10.0e0*ones(pb.n,1);
        pb.xupper = 10.0e0*ones(pb.n,1);
        %%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        pb.x0 = 8.0e0*ones(pb.n,1);
        pb.x0(ix_('X1'),1) = -8.0e0;
        pb.x0(ix_('X2'),1) = 8.0e0;
        %%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        ...
        [iv,ix_,pb] = s2xlib('nlx',vname,ix_,pb,1,-10.0e0,10.0e0,8.0e0);
