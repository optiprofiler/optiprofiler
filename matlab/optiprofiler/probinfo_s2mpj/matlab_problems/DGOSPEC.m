function varargout = DGOSPEC(action,varargin)
...
switch(action)
    ...
    case {'setup','setup_redprec'}
        ...
        vname = 'X1';
        [iv,ix_,pb] = s2mpjlib('nlx',vname,ix_,pb,1,-1.0,0.5,0.0);
        posev = find(strcmp('X',elftv{ielftype(ie)}));
        pbm.elvar{ie}(posev) = iv;
        [~,posep] = ismember('F',elftp{ielftype(ie)});
        pbm.elpar{ie}(posep) = v_('FREQ');
        %%%%%%%%%%%%%%%%%%%%%% GRFTYPE %%%%%%%%%%%%%%%%%%%%
...
% **********************
    ...
    case 'eCOS'
        ...
        varargout{1} = cos(FX);
        if(nargout>1)
            g_(1,1) = -pbm.elpar{iel_}(1)*sin(FX);
            varargout{2} = g_;
            if(nargout>2)
