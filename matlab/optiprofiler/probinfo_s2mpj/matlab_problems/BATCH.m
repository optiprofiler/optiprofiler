    ...
    case {'setup','setup_redprec'}
        ...
        for I=v_('1'):v_('N')
            for J=v_('1'):v_('M')
                ...
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['V',int2str(J)]);
                valA(end+1) = 1.0;
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['B',int2str(I)]);
                valA(end+1) = -1.0;
            end
        end
        for I=v_('1'):v_('N')
            for J=v_('1'):v_('M')
                ...
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['N',int2str(J)]);
                valA(end+1) = 1.0;
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['TL',int2str(I)]);
                valA(end+1) = 1.0;
            end
        end
        ...
        for J=v_('1'):v_('M')
            for K=v_('1'):v_('NU')
                ...
                irA(end+1)  = ig;
                icA(end+1)  = ix_(['Y',int2str(K),',',int2str(J)]);
                valA(end+1) = v_(['LOGI',int2str(K)]);
            end
            [ig,ig_] = s2mpjlib('ii',['NPAR',int2str(J)],ig_);
            gtype{ig}  = '==';
            cnames{ig} = ['NPAR',int2str(J)];
