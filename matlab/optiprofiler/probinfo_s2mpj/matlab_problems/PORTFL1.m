function varargout = PORTFL1(action,varargin)
        ...
        for I=v_('1'):v_('NR')
            for J=v_('1'):v_('NS')
                ...
                if(size(pbm.A,1)>=ig&&size(pbm.A,2)>=iv)
                    pbm.A(ig,iv) = v_(['F',int2str(J),',',int2str(I)])+pbm.A(ig,iv);
                else
                    pbm.A(ig,iv) = v_(['F',int2str(J),',',int2str(I)]);
                end
            end
        end
        for I=v_('1'):v_('NS')
