...
%         ``norm(A * inv - eye(n), 'fro') <= 1e-8 * n``. If `A` and
%         `inv` are both diagonal to roundoff, the bounds stay bounds;
%         otherwise every finite bound is posed as a linear constraint. A
%         bound is never dropped: since `mod_linear_ub` and `mod_linear_eq`
%         replace the linear constraints, supplying one of them with a
%         transformation that is not diagonal raises an error if the problem
%         has such bounds, unless `mod_bounds` is supplied as well.
...
function [solver_scores, profile_scores, curves] = benchmarkImpl(eval_report, varargin)
    ...
    if is_load
        ...
        if isfield(profile_options, ProfileOptionKey.SOLVER_ISRAND.value) ...
                && numel(profile_options.(ProfileOptionKey.SOLVER_ISRAND.value)) ~= n_solvers_loaded_eff
            error("MATLAB:benchmark:solver_israndAndLoadedSolversLengthNotSame", "The option `solver_isrand` must have the same length as the number of loaded solvers (%d).", n_solvers_loaded_eff);
        end
    ...
    reference_context = struct();
    if ~is_load
        stamp_origin = 'generated';
        if exist('options_user', 'var') && isfield(options_user, 'feature_stamp'), stamp_origin = 'explicit'; end
        feature_context = struct('feature_stamp', profile_options.feature_stamp, ...
            'full_feature_stamp', full_feature_stamp, 'feature_stamp_origin', stamp_origin);
        % Invocation provenance records supplied keywords, not a default that
        % happens to normalize through the same Feature constructor.
        if structured_feature
            feature_context.route = 'feature';
        elseif exist('options_user', 'var') && isfield(options_user, 'feature_name')
            feature_context.route = 'feature_name';
        end
        % A reference is generated by run_plain, never another user declaration
        % or the primary's explicitly overridden stamp.
        reference_context = struct('feature_stamp', 'plain', ...
            'full_feature_stamp', 'plain', 'feature_stamp_origin', 'generated');
    end
    if ~isempty(eval_report)
        eval_report.configure(problem_options, profile_options, feature, @prepareEvalReportHistory, ...
            primary_plan, reference_plan, feature_context);
    end
    ...
    if ~profile_options.(ProfileOptionKey.SCORE_ONLY.value)
        try
            if exist('options_user', 'var')
                ...
                if ~is_load
                    options_refined = optiprofiler_internal.refinedFeatureOptions( ...
                        feature, primary_plan, problem_options, profile_options, feature_context);
                    save(fullfile(path_log, 'options_refined.mat'), 'options_refined');
                    addToReadme(path_readme_log, 'options_refined.mat', 'Versioned native effective settings; use loadBenchmarkOptions for trusted fresh replay.');
                end
                % A replot cannot reconstruct unretained old feature/experiment
                % settings. options_user above records this load invocation;
                % do not publish a scratch plain configuration as fresh replay.
            end
        catch ME
    ...
    if exist('problem', 'var')
        ...
        if isstruct(result) && isfield(result, 'fun_history')
            result.feature_pipeline = optiprofiler_internal.EvalReport.encodeMetadata( ...
                optiprofiler_internal.featureProvenance(feature, primary_plan, feature_context));
        end
        if ~isempty(eval_report)
    ...
    if ~is_load
        ...
        for i_plib = 1:numel(plibs)
            ...
            results_plib = solveAllProblems(solvers, library, feature, problem_options, profile_options, true, path_hist_plots_plib, eval_report, 'primary', primary_plan, feature_context);
            ...
            if profile_options.(ProfileOptionKey.RUN_PLAIN.value)
                ...
                results_plib_plain = solveAllProblems(solvers, library, feature_plain, problem_options, profile_options, false, {}, eval_report, 'plain_reference', reference_plan, reference_context);
