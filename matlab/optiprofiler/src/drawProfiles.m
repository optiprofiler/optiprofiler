function [fig_perf_hist, fig_perf_ret, fig_data_hist, fig_data_ret, fig_log_ratio_hist, fig_log_ratio_ret] = drawProfiles(work_hist, work_ret, problem_dimensions, labels, tolerance_label, cell_axs_summary, is_perf, is_data, is_log_ratio)
%DRAWPROFILES draws the performance, data, and log-ratio profiles with respect to the whole history of function values or the returned values from solvers, respectively.
    ...
    elseif is_perf && is_log_ratio
        ...
        copy_work_ret2 = work_ret;
        drawLogRatioProfiles(cell_axs_summary{4}, copy_work_ret2, labels);
        % Set x-axis labels.
    ...
    elseif is_data && is_log_ratio
        ...
        drawPerformanceDataProfiles(cell_axs_summary{2}, x_data_ret, y_data_ret, labels);
        % Set x-axis limits.
        set(cell_axs_summary{1}, 'XLim', [0.0, 1.1 * ratio_max_data_hist]);
        set(cell_axs_summary{2}, 'XLim', [0.0, 1.1 * ratio_max_data_ret]);
        % Modify x-axis ticks labels of the data profiles.
        set(cell_axs_summary{1}, 'XTickLabel', xtl_data_hist, 'TickLabelInterpreter', 'latex');
        set(cell_axs_summary{2}, 'XTickLabel', xtl_data_ret, 'TickLabelInterpreter', 'latex');
        % Set x-axis labels.
    ...
    elseif is_log_ratio
        ...
        copy_work_ret2 = work_ret;
        drawLogRatioProfiles(cell_axs_summary{2}, copy_work_ret2, labels);
        % Set x-axis labels.
