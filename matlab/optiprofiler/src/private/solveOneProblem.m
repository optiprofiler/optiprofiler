function result = solveOneProblem(solvers, problem, feature, problem_name, len_problem_names, profile_options, is_plot, path_hist_plots, capture_presentation, experiment_plan)
%SOLVEONEPROBLEM solves one problem with all the solvers in solvers list.
%   The input is an ordinary Problem; one FeaturedProblem records each actual
%   solver invocation. The controller supplies the already resolved role plan.
    ...
    if isfield(profile_options, 'solver_log_names')
        solver_log_names = profile_options.solver_log_names;
    else
        solver_log_names = solver_names;
    end
    solver_log_names = cellfun(@(s) strrep(s, '\_', '_'), solver_log_names, 'UniformOutput', false);
    result = struct();
    ...
    % The controller resolved policy once for this role. A planned call is not
    % an observation; real_n_runs is updated only when a solver is invoked.
    planned_runs = experiment_plan.planned_runs;
    real_n_runs = zeros(n_solvers, 1);
    runtime_receipts = cell(n_solvers, n_runs);

    len_solver_log_names = max(cellfun(@length, solver_log_names));

    for i_solver = 1:n_solvers
        for i_run = 1:planned_runs(i_solver)
            if ~profile_options.(ProfileOptionKey.SILENT.value)
                fprintf('\n');
                format_info_start = sprintf("Start  solving    %%-%ds with %%-%ds (run %%2d/%%2d).", len_problem_names, len_solver_log_names);
                printOptiProfilerMessage('INFO', sprintf(format_info_start, problem_name, solver_log_names{i_solver}, i_run, planned_runs(i_solver)));
            end
            ...
            try
                try
                    real_n_runs(i_solver) = real_n_runs(i_solver) + 1;
                    ...
                    if profile_options.(ProfileOptionKey.SOLVER_VERBOSE.value) ~= 0
                        printOptiProfilerMessage('WARNING', sprintf('An error occurred while solving %s with %s (run %d/%d): %s', problem_name, solver_log_names{i_solver}, i_run, planned_runs(i_solver), shortenMessageForLog(Exception.message)));
                    end
                end
                ...
                if elapsed_s < 0
                    ...
                    if ~profile_options.(ProfileOptionKey.SILENT.value)
                        printOptiProfilerMessage('WARNING', sprintf('Negative elapsed time for %s with %s (run %d/%d): t_start=%.6f t_end=%.6f diff=%.6g s (clamped to 0; see https://www.mathworks.com/matlabcentral/answers/97194).', problem_name, solver_log_names{i_solver}, i_run, planned_runs(i_solver), time_start_posix, time_end_posix, elapsed_s));
                    end
                    elapsed_s = 0;
                elseif ~isfinite(elapsed_s) || elapsed_s > 1e7
                    ...
                    if ~profile_options.(ProfileOptionKey.SILENT.value)
                        printOptiProfilerMessage('WARNING', sprintf('Implausibly large elapsed time %.6g s for %s with %s (run %d/%d): t_start=%.6f t_end=%.6f (recorded as NaN; see https://www.mathworks.com/matlabcentral/answers/97194).', elapsed_s, problem_name, solver_log_names{i_solver}, i_run, planned_runs(i_solver), time_start_posix, time_end_posix));
                    end
                    elapsed_s = NaN;
                end
                ...
                if ~profile_options.(ProfileOptionKey.SILENT.value)
                    format_info_end = sprintf("Finish solving    %%-%ds with %%-%ds (run %%2d/%%2d) in %%.2f seconds.", len_problem_names, len_solver_log_names);
                    printOptiProfilerMessage('INFO', sprintf(format_info_end, problem_name, solver_log_names{i_solver}, i_run, planned_runs(i_solver), computation_time(i_solver, i_run)));
                    switch problem_type
                        case 'u'
                            format_info_output = sprintf("Output result for %%-%ds with %%-%ds (run %%2d/%%2d): f = %%10.4e.", len_problem_names, len_solver_log_names);
                            printOptiProfilerMessage('INFO', sprintf(format_info_output, problem_name, solver_log_names{i_solver}, i_run, planned_runs(i_solver), fun_out(i_solver, i_run)));
                            format_info_best = sprintf("Best   result for %%-%ds with %%-%ds (run %%2d/%%2d): f = %%10.4e.", len_problem_names, len_solver_log_names);
                            printOptiProfilerMessage('INFO', sprintf(format_info_best, problem_name, solver_log_names{i_solver}, i_run, planned_runs(i_solver), fun_min));
                        otherwise
                            format_info_output = sprintf("Output result for %%-%ds with %%-%ds (run %%2d/%%2d): f = %%10.4e, maxcv = %%10.4e.", len_problem_names, len_solver_log_names);
                            printOptiProfilerMessage('INFO', sprintf(format_info_output, problem_name, solver_log_names{i_solver}, i_run, planned_runs(i_solver), fun_out(i_solver, i_run), maxcv_out(i_solver, i_run)));
                            format_info_best = sprintf("Best   result for %%-%ds with %%-%ds (run %%2d/%%2d): f = %%10.4e, maxcv = %%10.4e.", len_problem_names, len_solver_log_names);
                            printOptiProfilerMessage('INFO', sprintf(format_info_best, problem_name, solver_log_names{i_solver}, i_run, planned_runs(i_solver), fun_min, maxcv_min));
                    end
                end
                ...
                if profile_options.(ProfileOptionKey.SOLVER_VERBOSE.value) ~= 0
                    printOptiProfilerMessage('WARNING', sprintf('An error occurred while processing the solution of %s from %s (run %d/%d): %s', problem_name, solver_log_names{i_solver}, i_run, planned_runs(i_solver), shortenMessageForLog(Exception.message)));
                end
                % Clear the solution.
                clear x;
            end
