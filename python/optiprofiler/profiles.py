...
def _benchmark(
    ...
    **kwargs
    ...
    mod_affine : callable, optional
        ...
        ``norm(max(abs(A @ inv - I) - 64 * n * eps * abs(A) @ abs(inv), 0), 'fro') <= 1e-8 * n``
        ...
        rows then columns of ``A`` are scaled by powers of two to form ``E`` without
        losing any entries, and ``cond(E, 1) < 1 / eps`` is required. These
        numerical checks are not a proof that ``inv`` is an exact inverse.
        Integer and extended precision inputs are rounded to double precision
        before validation and use. If ``A`` is exactly diagonal and
        ...
        off-diagonal entry of ``A`` is never ignored, however small. Transport
        rejects finite bounds, coefficients or right-hand sides becoming
        non-finite, nonzero scaled bounds below the smallest normal magnitude
        (about 2.2e-308), and row or right-hand-side entries whose sum of
        absolute nonzero product terms lies below that magnitude. A strict
        original bound interval must not collapse to equal endpoints after
        translation or diagonal scaling.
        ...
        and ``sqrt(eps) * minimum(abs(x0), abs(x0 - b))``, componentwise.
        ...
        solve instead of trusting the inverse candidate; it is not a bound on
        the final forward error. Even an honestly computed inverse may trigger
        a solve, for example when a custom rotation mixes small and large coordinates.
        ...
        or built-object pickles; loading is not a migration of old results.
    mod_bounds : callable, optional
        ...
        representation. When ``mod_affine`` uses generated bound rows, the
        framework therefore does not add that original box again as linear
