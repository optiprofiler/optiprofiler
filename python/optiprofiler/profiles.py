...
def _benchmark(
    ...
    **kwargs
    ...
    mod_affine : callable, optional
        ...
        the products it is summed from, and nothing more, so that neither the
        units of the variables nor the condition number decide, and an error
        of ``inv`` that is not rounding is refused. Integer and extended
        precision arrays are converted to double precision, which rounds to
        nearest as every decimal literal is rounded; the rounded triple is the
        one that is validated and used. If ``A`` is exactly diagonal and
        ...
        normal number, 2.2e-308, or an entry of a transported row or
        right-hand side whose terms are all below it), raises an error instead
        of being posed as infinite or as zero. The initial point is
        ``inv @ (x0 - b)`` only if ``A`` maps it back to ``x0`` to the rounding
        of that evaluation, in every component; otherwise ``A @ y = x0 - b``
        is solved, and if that point is not mapped back either (overflow,
        underflow), an error is raised: no tolerance on ``inv`` bounds an
        error at a point. Since ``mod_linear_ub`` and ``mod_linear_eq`` replace
        ...
        unless ``mod_bounds`` is supplied as well. The derivative methods of a
        featured problem follow the same transformation by the chain rule
        (see `FeaturedProblem`).
